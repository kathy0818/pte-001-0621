...
>
> 👉 **要动手做新一套（新考试、新截图、新成绩图）：先读 [`制作流程.md`](制作流程.md)（总册：安装、命名/版本约定、通用流程、读图技巧、LaTeX 工具箱）**，
> 再按部分看分册：[`制作流程_写作.md`](制作流程_写作.md)、[`制作流程_口语.md`](制作流程_口语.md)。本 README 偏总览与规则。
...
├─ 制作流程_写作.md  制作流程_口语.md   ← 分册：各部分的细节做法
│
...
│   ├─ 001_PTE模考1B_写作_整理卷.{pdf,tex}     （每个 part 各自从 000 起）
│   └─ 000_PTE模考1B_口语_整理卷/              含图片版本 = 子文件夹
│       ├─ 000_PTE模考1B_口语_整理卷.{pdf,tex}
│       └─ img/                                （DI 配图等）
│
...
3. **每个 part 各自从 `000` 起编号**（写作 000、001…；口语 000、001…，互不干扰），每更新一版 **+1**（三位数）。某 part 最新版 = 它名字下最大的序号。
4. **绝不修改、绝不覆盖任何旧版本**——只**新增**下一个序号。旧的永远保持原样。
5. **纯文字版本**：`.tex` 单文件自包含（内联样式、无外部图），直接放 `整理卷/` 下（写作即如此）。
   **含图片版本**（如口语 DI 配图）：做成一个**子文件夹** `整理卷/<序号>_…_整理卷/`，里面放 `.tex` + `img/` + `.pdf`，「tex + 图片」整体自包含。两者都用 `build/flatten.py` 生成（含图片时多传一个图片目录名，见 §6）。
...
4. **产出新版本**：`python3 build/flatten.py <part英文> <中文> <考试号> <下一个序号> [图片目录]`。无图→单文件 `.tex` 进 `整理卷/`；有图（把图放 `build/paper/<图片目录>/`，正文用 `\includegraphics{<图片目录>/x.png}`）→ 生成子文件夹（tex+img+pdf）。再到目标处 `xelatex`——**不动任何旧版本**（见 §4）。
