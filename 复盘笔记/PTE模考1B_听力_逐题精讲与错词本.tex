...
\end{enumerate}
\ptesub{高分范文（搬原文，自己只加连接词）}
\begin{modelbox}\small
Education stands as a powerful tool in bridging social inequalities and shaping a more equitable society. \textbf{However,} access to quality education is unevenly distributed, influenced by socioeconomic status, race, and gender. The digital divide further disadvantages students in remote areas. \textbf{Nevertheless,} initiatives such as scholarship programs and digital learning platforms foster critical thinking and empower individuals to rise above their circumstances.
\end{modelbox}
{\footnotesize\color{gray}（61 词，安全落在 50–70。加粗的 However / Nevertheless 及若干 and 是仅有的"自己的话"，其余约 90\% 照搬录音原句——语法、词汇天然干净，不用自己改写。）}
\ptesub{怎么听 · 记笔记取舍（记骨架，不记肉）}
\begin{notebox}\small
一遍过、记不全也无妨。\textbf{该记：}主题、每段中心句、转折(however/but)后、因果结论(therefore/in conclusion)后、反复出现的关键名词。
\textbf{要丢：}\textbf{such as / for example / including 后面的例子}、数字年份、重复换说法、修饰废话。\\[2pt]
\textbf{本篇只需记这 4 行骨架}（录音原词 + 符号）：\\
\emph{1. edu = tool $\to$ bridge inequality / equitable society}\\
\emph{2. BUT access unequal $\leftarrow$ socioeconomic · race · gender}\\
\emph{3. digital divide $\to$ remote/poor students disadvantaged}\\
\emph{4. HOPE: initiatives $\to$ critical thinking + empower}\\[2pt]
\textbf{铁律：}记例子的代价不能是漏掉下一个主点；一篇记 3–5 行就够（对应 3–4 个主点）。
\end{notebox}
\ptesub{怎么写 · 模板填法（模板=骨架，填空=搬原文）}
\begin{notebox}\small
保底模板（语法/字数焊死，填空塞录音原词）：\\
\emph{The lecture discusses [主题]. It points out that [主点1（要有主谓）]. The speaker also mentions [主点2/原因]. Finally, the speaker suggests [解决/结论].}\\[2pt]
\textbf{三个雷别踩：}\textbf{(1)} 第 4 句别用 ``are introduced''（单复数陷阱），改主动 ``the speaker suggests / introduces''；
\textbf{(2)} ``points out / explains that'' 后面必须跟\textbf{完整句子}（主+谓），不能填名词；
\textbf{(3)} 框架词不得分，\textbf{得分全靠填空里的录音原词}。
\end{notebox}
\ptesub{拼写要盯 · 下次不会错}
\begin{notebox}\small
\textbf{拼写要盯：}inequalities · equitable · unevenly · distributed · socioeconomic · scholarship · circumstances。\\[2pt]
\yes\ \textbf{下次不会错：}不可数名词（education/society/research）泛指不加 the；交卷两遍校对——先查冠词、再查动词单复数。\\[2pt]
\textbf{口诀：模板保语法字数，搬原文拿内容词汇。}
\end{notebox}
\end{qblock}
